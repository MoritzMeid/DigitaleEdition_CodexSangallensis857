<?xml version="1.0" encoding="UTF-8"?>
          ...
          --><xsl:param name="mainPrefix" select="'.'"/><xsl:param name="filePrefix" select="'../../..'"/><xsl:param name="dataPrefix" select="'../..'"/><xsl:param name="imageExt" select="'jpg'"/><!-- EN: Main web site for digital edition -->
        <!-- IT: Sito web principale dell'edizione digitale -->
        <!-- default: '' --><xsl:param name="webSite" select="''"/><!-- EN: Edition title -->
        <!-- IT: Titolo edizione -->
        <!-- default: 'Codex Viewer' --><xsl:param name="index_title" select="'St. Galler Epenhandschrift - Codex Sangallensis 857'"/><!-- EN: Welcome Message -->
          <!-- IT: Messaggio di benvenuto -->
          <!-- default: 'Welcome to an edition created with EVT' --><xsl:param name="welcomeMsg">
		<div xmlns="http://www.w3.org/1999/xhtml">
			<p class="title main">Welcome to an edition created with EVT!</p>
		</div>
		<div xmlns="http://www.w3.org/1999/xhtml">This archive includes a few examples of editions created using EVT,
		...
		<div xmlns="http://www.w3.org/1999/xhtml">It is recommended to go to full screen mode so that all available screen space is used to show the manuscript images and the transcription text.</div>
		<div xmlns="http://www.w3.org/1999/xhtml">For more information refer to the EVT Manual in the "doc" folder.
	...
	</xsl:param><!-- EN: Hide/Show scans -->
        <!-- IT: Nascondi/Mostra scansioni -->
        <!-- default: true() --><xsl:param name="image_frame" select="true()"/><!-- EN: Hide/Show badge -->
        ...
        <!-- IT: E' possibile personalizzare il prefisso usato nella creazione delle classi degli elementi html dell'edizione. --><xsl:variable name="ed_name1">dipl</xsl:variable><xsl:variable name="ed_name2">interp</xsl:variable><!-- EN: Enable/Disable translation -->
          <!-- IT: Attiva/Disattiva traduzione --><xsl:param name="translation" select="true()"/><!-- Thumbnail image --><xsl:param name="fb_thumb" select="'thumb_fb.jpg'"/><!-- EN: Indicate the xml node that contains all the text to be transformed for each edition level -->
        ...
        --><xsl:param name="left_frame_default_content" select="'image'"/><!-- EN: Set default content on first load for right frame choosing between text or text front info
        Possible values are:
        ...
        <!-- default: true() --><xsl:param name="thumbs_button" select="true()"/><!-- EN: Show/Hide Viscoll Button in interface -->
          ...
          <!-- default: false() --><xsl:param name="viscoll_button" select="true()"/><!-- VISCOLL -->
            ...
            <!-- default: true() --><xsl:param name="pp_selector_doc_grouping" select="true()"/><!-- EN: Choose whether or not having a tooltip on pages option showing the belonging document  -->
              ...
              <!-- default: false() --><xsl:param name="pp_selector_doc_tooltip" select="true()"/><!-- EN: On/Off Search -->
                ...
                <!-- default: true() --><xsl:param name="search" select="true()"/><!-- EN: On/Off Virtual Keyboard for search -->
                  ...
                  <!-- default: true() --><xsl:param name="virtual_keyboard_search" select="true()"/><!-- EN: On/Off Bottom Navbar -->
                    <!-- IT: Attiva/Disattiva Barra di navigazione in fondo alla pagina -->
                    <!-- default: true() --><xsl:param name="bottom_navbar" select="true()"/><!-- EN: Initial status of Bottom Navbar (only works if $bottom_navbar=true()) -->
        <!-- IT: Stato iniziale della barra di navigazione (considerato solo se $bottom_navbar=true())-->
        <!-- possible values: 'collapsed' | 'expanded' -->
        <!-- default: 'collapsed' --><xsl:param name="bottom_navbar_initial_status" select="'expanded'"/><!-- EN: On/Off Document Navigation -->
          ...
          <!-- default: false() --><xsl:param name="document_navigation" select="true()"/><!-- EN: On/Off Lines/Prose visualization Button -->
            ...
            <!-- default: true() --><xsl:param name="prose_verses_toggler" select="true()"/><!-- EN: On/Off Tooltip indicating language encoded with @xml:lang -->
              ...
              <!-- default: false() --><xsl:param name="lang_tooltip" select="true()"/>
              <!-- LISTS -->
		...
		<p xmlns="http://www.w3.org/1999/xhtml">To ensure that it will be working on all the most recent web browsers, and for as long as possible on the World
		...
		<p xmlns="http://www.w3.org/1999/xhtml">For more information about how to use and/or customize EVT please refer to the EVT Manual included in the
			archive you downloaded, in the "doc" folder.</p>
		<p xmlns="http://www.w3.org/1999/xhtml">EVT is used in the following projects:</p>
		...
		<p xmlns="http://www.w3.org/1999/xhtml">EVT has a home page <a href="http://evt.labcd.unipi.it/" target="blank">here</a> and an older one in the <a href="https://sourceforge.net/p/evt-project/" target="blank">SourceForge</a> repository, but development is done
		...
		<p xmlns="http://www.w3.org/1999/xhtml">If you have any suggestions or spot an error/bug please contact us at <a href="mailto:evt.developers@gmail.com">evt.developers@gmail.com</a></p>
